...
// ================= 2. FEED SCREEN (WITH PERMANENT CLOUD SYNC & DELETE) =================
...
class _TikTokCardState extends State<TikTokCard> with SingleTickerProviderStateMixin {
  ...
  // 🗑️ DELETE VIDEO ACTION (የራስን ቪዲዮ ማጥፊያ)
    ...
    return GestureDetector(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Right Side Action Icons + DELETE BUTTON FOR OWNER
          Positioned(
            bottom: 25,
            right: 12,
            child: Column(
              children: [
                ...
                // 🗑️ OWNER DELETE BUTTON (ባለቤቱ ከሆነ ብቻ የሚያየው ማጥፊያ)
                if (isOwner)
                  IconButton(
...
class _RealCameraStudioState extends State<RealCameraStudio> {
  ...
  Future<void> _uploadPostToCloud() async {
    ...
    // 1. ወደ מקומיው ዝርዝር መጨመር
    AppState.allPosts.insert(0, newPost);

    // 2. ወደ MongoDB ደመና ዳታቤዝ በቋሚነት መላክ
    try {
      await http.post(
...
// ================= 7. PROFILE & SETTINGS (WITH PHOTO CHANGE & DELETE GRID) =================
